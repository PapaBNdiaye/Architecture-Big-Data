...
const TempTrendChart: React.FC<TempTrendChartProps> = ({ data }) => {
  // Filtrer les données de température moyenne
  const tempData = data.filter(row => row.metric_name === 'avg_temp_c');

  // Transformer les données pour Recharts
  const chartData = tempData.reduce((acc: any[], row) => {
  ...
  const cities = Array.from(new Set(tempData.map(row => row.location)));

  if (chartData.length === 0) {
    return (
      <Card title="Évolution des températures moyennes">
  ...
  return (
    <Card title="Évolution des températures moyennes par ville">
